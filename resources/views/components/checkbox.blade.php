@php
    $colorClasses = match($color) {
        ...
        default   => 'checked:bg-slate-800 checked:border-slate-800',
    };
@endphp
...
<div class="inline-flex items-center">
    <label class="flex items-center cursor-pointer relative">
        <input 
            ...
            value="{{ $value }}"
            {{ $attributes->merge([
                'class' => "peer h-5 w-5 cursor-pointer transition-all appearance-none rounded shadow hover:shadow-md border border-slate-400 {$colorClasses}"
            ]) }}
